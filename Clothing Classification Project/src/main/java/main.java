...
//                .dropOut(0.2)
        ...
        vgg16Transfer = new TransferLearning.GraphBuilder(vgg16)
                ...
                //let's try to add a pooling layer n see
                //.addLayer("global_avg_pool", new GlobalPoolingLayer(PoolingType.AVG), "fc2")
                //ERROR: Exception in thread "main" java.lang.UnsupportedOperationException: Received rank 2 input (shape = [16, 1024]). Only rank 3 (time series), rank 4 (images/CNN
...
//        System.out.println(eval1.stats());
        ...
        log.info("Train with 10 Epochs");
        vgg16Transfer.fit(trainIter, 10);
        Evaluation eval2 = vgg16Transfer.evaluate(testIter);
        System.out.println(eval2.stats()); // first output forogt to print out stats zzz
        File modelFilenameTwo = new File(System.getProperty("user.home"), ".deeplearning4j/generated-models/second-model.zip");
        ModelSerializer.writeModel(vgg16Transfer, modelFilenameTwo, true);
    }
